                             ...
                             <form name="add_name" method="POST" action="/add-more-post">
                                ...
                                <div class="table-responsive" id="dynamic_field">  
                                    <table class="table table-bordered">  
                                        ...
                                        <tr>
                                            <td>Item Sampah</td>   
                                            <td><select id = "item_sampah" name="item_sampah" class="form-control" >
...
<script type="text/javascript">
    $(document).ready(function(){      
      ...
      $('#add').click(function(){  
           i++;  
           $('#dynamic_field').append('<div id="row'+i+'" class="table-responsive" id="dynamic_field"><table class="table table-bordered"><div class="ln_solid"></div><tr><td>Kategori Sampah</td><td><select id = "kategori" name="kategori" class="form-control" ><?php if( ! empty($kategori_sampah)){ // Jika data pada database tidak sama dengan empty (alias ada datanya)
                                                                  foreach($kategori_sampah as $data){ ?><option value="<?php echo $data->KODE_GENERAL_REF_DETAIL; ?>"><?php echo $data->DESCRIPTION; ?></option><?php } }else {
                                                                  echo "string";
                                                                } 
                                                          ?></select></td></tr><tr><td>Item Sampah</td><td><select id = "item_sampah" name="item_sampah" class="form-control" ><?php if( ! empty($item_sampah)){ // Jika data pada database tidak sama dengan empty (alias ada datanya)
                                                                    foreach($item_sampah as $data){ ?><option value="<?php echo $data->ISM_ID; ?>"><?php echo $data->ISM_NAMA; ?></option><?php } }else {
                                                                    echo "string";
                                                                  } 
                                                            ?></select></td></tr><tr><td>Berat</td><td><input type="number" id = "berat" name="berat" class="form-control name_list"/></td></tr></table><div class="ln_solid"></div><td><button type="button" name="remove" id="'+i+'" class="btn btn-danger btn_remove">Delete</button></td></div>');});
