...
class kurir_order extends CI_Controller {
	...
	{
		$query = $this->db->query("SELECT * FROM ORDERS_VIEW2 WHERE STATUS = 'Sudah Diambil'");
        $hasil = $query->result();
        ...
        $data['user'] = $this->db->get_where('USERS', ['USR_NAMA' => $this->session->userdata('username')])->row_array();
		...
		$this->load->view('Kurir/kurir_order', $data);
